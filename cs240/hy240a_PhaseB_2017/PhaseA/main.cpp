...
#define DPRINT(...)
	...
	*/
	...
	int general_resign(int gid1, int gid2) {
	    ...
	    patr_army = patroklos->soldiers_head;
	    while(ax_army != NULL && patr_army != NULL){
            if(ax_army->sid < patr_army->sid){
                DDL_soldier *new_soldier_to_rest = (DDL_soldier*)malloc(sizeof(DDL_soldier));
                new_soldier_to_rest->sid = ax_army->sid;
                if(patr_army != NULL) new_soldier_to_rest->next = patr_army;
                else new_soldier_to_rest->next = NULL;

                    if(patr_army->prev == NULL){
                        patroklos->soldiers_head = new_soldier_to_rest;
                        new_soldier_to_rest->prev = NULL;
                    }
                    else{
                        patr_army->prev->next = new_soldier_to_rest;
                        new_soldier_to_rest->prev = patr_army->prev;
                    }
            ...
            else if(ax_army->sid > patr_army->sid && patr_army->next == NULL){
                DDL_soldier *new_soldier_to_rest = (DDL_soldier*)malloc(sizeof(DDL_soldier));
                new_soldier_to_rest->sid = ax_army->sid;
                patr_army->next = ax_army;
                ax_army = ax_army->next;
            }
            if(ax_army->sid > patr_army->sid) patr_army = patr_army->next;
